...
function theme_tresipunt_get_pre_scss($theme) {
...
// Note the function name starts with the component name because this is a global function and we don't want namespace clashes.
function theme_tresipunt_get_extra_scss($theme) {
    // Load the settings from the parent.
    $theme = theme_config::load('boost');
...
function theme_tresipunt_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_tresipunt', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_tresipunt and not theme_boost (see the line above).                
